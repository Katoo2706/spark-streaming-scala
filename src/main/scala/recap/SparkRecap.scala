...
/**
 * Different method of loading data:
 * .format("json")
 ...
 *
 * .format("json")
 * .option("path", "src/main/resources/data/bands")
 * .load()*/
...
object SparkRecap {
  ...
  val spark = SparkSession.builder()
    .appName("Spark Recap")
    .master("local[2]") // 2 thread on machine
    .getOrCreate()
  ...
  def main(args: Array[String]): Unit = { // same as if __name__ == "__main__"
